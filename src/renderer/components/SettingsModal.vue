    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-[2px]" @click.self="$emit('close')">
      <div class="w-full max-w-md bg-gray-900 border border-gray-700 rounded-md shadow-2xl">
        ...
        <div class="px-6 py-5 space-y-6">
          ...
          <div>
            <p class="text-[10px] font-semibold text-gray-500 uppercase tracking-widest mb-4">{{ $t('settings.info') }}</p>
            <div class="space-y-5">

              <SettingRow :label="$t('settings.about.github')" :sub="$t('settings.about.githubSub')">
                <button
                  @click="openExternal('https://github.com/LeonardoCiaccio/Vorn')"
              ...
              <SettingRow :label="$t('settings.about.releases')" :sub="$t('settings.about.releasesSub')">
                <button
                  @click="openExternal('https://github.com/LeonardoCiaccio/Vorn/releases')"
                  class="flex items-center gap-1.5 text-xs text-indigo-400 hover:text-indigo-300 transition-colors cursor-pointer"
                >

      ...
      <div class="w-full max-w-lg bg-gray-900 border border-gray-700 rounded-lg shadow-2xl flex flex-col overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-800 flex items-center justify-between shrink-0">
          <div class="flex items-center gap-3">
            ...
            <h2 class="text-sm font-bold text-white">Estrai files dallo store</h2>
          </div>
        ...
        <div class="px-6 py-5 space-y-5 flex-1 overflow-y-auto">
          ...
          <div class="bg-amber-950/30 border border-amber-900/40 rounded-md px-4 py-3 space-y-1.5">
            <div class="flex items-center gap-2">
              ...
              <p class="text-xs font-semibold text-amber-300">Procedura di recupero dati</p>
            </div>
            <p class="text-[11px] text-amber-200/70 leading-relaxed">
              Usa questa funzione <strong>solo se hai perso i riferimenti delle sessioni</strong>. I file estratti potrebbero non corrispondere alle sessioni originali: nel tempo i record potrebbero essere stati modificati, file eliminati o spostati.
            </p>
            <p class="text-[11px] text-amber-200/70 leading-relaxed">
              Se esistono più sessioni con lo stesso nome, verranno create cartelle separate nel formato <span class="font-mono bg-amber-900/30 px-1 rounded">nome-id</span> — ad esempio <span class="font-mono bg-amber-900/30 px-1 rounded">Pippo-a1b2c3</span> e <span class="font-mono bg-amber-900/30 px-1 rounded">Pippo-d4e5f6</span>.
            </p>
          </div>

          <!-- Filtro sessione -->
          <div>
            <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Filtra per sessione</label>
            <p class="text-[11px] text-gray-600 mb-2">Lascia vuoto per estrarre tutti i file da tutte le sessioni trovate nello store.</p>
            <input
          ...
          <!-- Destinazione -->
          <div>
            <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Cartella di destinazione</label>
            <div class="flex items-center gap-2">
              <input
                v-model="destDir"
                :disabled="running"
            ...
            <p class="text-[11px] text-gray-600 mt-1.5">I file saranno estratti in <span class="font-mono">destinazione/&lt;sessione-id&gt;/&lt;percorso relativo&gt;</span></p>
          </div>

          <!-- Progress -->
          <div v-if="running" class="space-y-2">
            <div class="flex items-center justify-between text-xs">
              <span class="text-gray-400">Analisi in corso…</span>
            ...
            <div class="flex items-center gap-4 text-[11px] text-gray-500">
              <span class="text-emerald-400">{{ progress.extracted ?? 0 }} estratti</span>
              <span v-if="progress.errors" class="text-red-400">{{ progress.errors }} errori</span>
            </div>
          </div>
          ...
          <div v-if="result && !running" class="space-y-3">
            <div class="flex items-center gap-2 text-sm" :class="result.fatalError ? 'text-red-400' : 'text-emerald-400'">
              ...
              <span v-if="result.fatalError">Errore: {{ result.fatalError }}</span>
              <span v-else-if="result.total === 0">Store vuoto — nessun file da estrarre</span>
              <span v-else>{{ result.extracted }} file estratti su {{ result.total }} analizzati</span>
            </div>

            <!-- Sessioni trovate -->
            <div v-if="result.sessions?.length" class="bg-gray-800/50 border border-gray-800 rounded-md px-4 py-3">
              <p class="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-2">Sessioni trovate nello store</p>
            ...
            <div v-if="result.noRecords?.length" class="bg-amber-950/30 border border-amber-900/40 rounded-md px-4 py-3">
              <p class="text-[11px] font-semibold text-amber-500 uppercase tracking-wider mb-1">File senza intestazione ({{ result.noRecords.length }})</p>
              <p class="text-[11px] text-gray-500">Questi file non contengono records di sessione e non sono stati estratti.</p>
            </div>

            <!-- Errori -->
            <div v-if="result.errors?.length" class="bg-red-950/20 border border-red-900/40 rounded-md px-4 py-3">
              <p class="text-[11px] font-semibold text-red-400 uppercase tracking-wider mb-2">{{ result.errors.length }} errori</p>
...
async function start() {
  submitError.value = ''
  if (!destDir.value.trim()) { submitError.value = 'Seleziona una cartella di destinazione'; return }
  try {
    await startExtractStore(destDir.value.trim(), sessionFilter.value.trim() || null)
  } catch (e) {
    submitError.value = e.message ?? 'Errore durante l\'estrazione'
  }
}
